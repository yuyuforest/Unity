...
public class GameController : NetworkBehaviour, SceneController, IUserAction
{
    ...
    public override void OnStartServer()
    {
        ...
        //加载GUI
        GameObject gui = Instantiate<GameObject>(Resources.Load<GameObject>("Prefabs/GUI"), Vector3.zero, Quaternion.identity);
        NetworkServer.Spawn(gui);
    }
    ...
    public void LoadMap()
    {
        ...
        walls[5] = Instantiate<GameObject>(Resources.Load<GameObject>("Prefabs/Walls/Wall4_3"), new Vector3(12.5f, 0.5f, 7.5f), Quaternion.identity);


        for (int i = 0; i < 6; i++) NetworkServer.Spawn(walls[i]);
    }
    ...
    public void LoadCharacters()
    {
        ...
        patrols[3] = patrolFactory.GetPatrol(new Vector3(4, 0.5f, 8), Quaternion.AngleAxis(180, Vector3.up));
        patrols[4] = patrolFactory.GetPatrol(new Vector3(8, 0.5f, 7), Quaternion.AngleAxis(90, Vector3.up));
        patrols[5] = patrolFactory.GetPatrol(new Vector3(11, 0.5f, 8.5f), Quaternion.identity);

        for (int i = 0; i < 6; i++) NetworkServer.Spawn(patrols[i]);
    }
